...
```
If you need an actual string rather than a React.Node, as for an `aria-label` attribute, use the [injectIntl HOC](https://formatjs.io/docs/react-intl/api#injection-api) or `stripes-core`'s `<IntlConsumer>`:
```
